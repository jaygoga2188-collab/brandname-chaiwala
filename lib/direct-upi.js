...
function buildDirectUpiLinks({ amount, transactionReference }, config) {
  ...
  // Exact iPhone format from the supplied Payment.jsx: pa, pn, am, cu, tn.
  // Do not append Android/QR-only tr to the iPhone PhonePe app URI.
  const phonePeUri = `phonepe:upi://pay?pa=${config.upiId}&pn=${encodeURIComponent(config.payeeName)}&am=${Number(amount)}&cu=INR&tn=${encodeURIComponent(transactionReference)}`;
  ...
  return {
    ...
    phonePeUri,
    androidIntentUri: `intent://pay?${query}#Intent;scheme=phonepe;package=com.phonepe.app;end`,
